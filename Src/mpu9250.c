...
extern float a[3], g[3], m[3]; // variables to hold latest sensor data values
extern __IO bool i2c1_mem_rx_irq_flag;
...
#define A_DLPF_CFG_460Hz    0x00 // Hz is for bandwidth, not rate (the rate is 1kHz)
#define A_DLPF_CFG_184Hz    0x01
#define A_DLPF_CFG_92Hz     0x02
#define A_DLPF_CFG_41Hz     0x03

#define GYRO_FCHOICE        0x03 // to enable digital low-pass filter (bit 0 = 1, and bit 1 = 1)
#define GYRO_DLPF_CFG_250HZ 0x00 // Hz is for bandwidth, not rate (the rate is 1kHz)
...
float GyroMeasError = 0.698132; //PI * (40.0f / 180.0f);   // gyroscope measurement error in rads/s (start at 40 deg/s)
float GyroMeasDrift = 0; //PI * (0.0f  / 180.0f);   // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)
...
float beta = 0.604600; //sqrt(3.0f / 4.0f) * GyroMeasError*0.1;   // compute beta
float zeta = 0; //sqrt(3.0f / 4.0f) * GyroMeasDrift;   // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value
// these are the free parameters in the Mahony filter and fusion scheme, Kp for proportional feedback, Ki for integral
#define Kp 25.0f
#define Ki 0.25f

uint8_t readByte(uint8_t i2CAddr, uint8_t regAddr)
{
	uint8_t singleByte[] = {0};
	HAL_I2C_Mem_Read(&hi2c1, i2CAddr<<1, regAddr, 1, &singleByte, 1, HAL_MAX_DELAY);
	return singleByte[0];
}

void readBytes(uint8_t i2CAddr, uint8_t regAddr, uint16_t numBytes, uint8_t *pData)
{
	HAL_I2C_Mem_Read(&hi2c1, i2CAddr<<1, regAddr, 1, pData, numBytes, HAL_MAX_DELAY);
	return;
}

void writeByte(uint8_t i2CAddr, uint8_t regAddr, uint8_t data)
{
	uint8_t singleByte[] = {data};
	HAL_I2C_Mem_Write(&hi2c1, (i2CAddr<<1)|1, regAddr, 1, &singleByte, 1, HAL_MAX_DELAY);
	return;
}
...
void readAccelData(float *accel_vec) //
{
  ...
  HAL_I2C_Mem_Read_DMA(&hi2c1, 0x68<<1, ACCEL_XOUT_H, 1 , rawData, sizeof(rawData));
...
void readGyroData(float *gyro_vec)
{
  ...
  HAL_I2C_Mem_Read_DMA(&hi2c1, 0x68<<1, GYRO_XOUT_H, 1 , rawData, sizeof(rawData));
  i2c1_mem_rx_irq_flag = false;
  while(!i2c1_mem_rx_irq_flag);
...
// #######################################################################################################
void setup_mpu9250(int accel_range, int gyro_range, int mag_bits, int gyro_dlpf, int accel_dlpf)
{
  bool verbose = false;
  char dummy_str[50];
  ...
  switch (gyro_range)  {
    ...
    default:
      printf("[ERROR]: Bad Gyro range command: ");
      printf("gyro_range\n\r");
  }
  if (verbose) {
    printf("[Setup] Setting Gryo command:");
    sprintf(dummy_str, " %x (hex) =  %d (dec) \n\r", gyro_range_cmd, gyro_range_cmd);
    printf(dummy_str);
  }
  ...
  switch (accel_range)  {
    ...
    default:
      printf("[ERROR]: Bad Accelerometer range command");
      printf("%d\n\r",accel_range_cmd);
  }
  if (verbose) {
    printf("[Setup] Setting Accelerometer command:");
    sprintf(dummy_str, " %x (hex) =  %d (dec) \n\r", accel_range_cmd, accel_range_cmd);
    printf(dummy_str);
  }
  ...
  switch (mag_bits)  {
    ...
    default:
      printf("[ERROR] Bad Mag Resolution command\n\r");
      printf("%d\n\r",mag_bits);
  }
  ...
  if (SerialDebug){
    printf("MPU9250 "); printf("I AM "); printf("0x%X",c); printf(" I should be "); printf("0x%X\n\r",0x71);
  }
  ...
  if (c == 0x71) // WHO_AM_I should always be 0x71
  {
    if (SerialDebug){
      ...
      printf("x-axis self test: acceleration trim within : "); printf("%0.1f",SelfTest[0]); printf("%% of factory value\n\r");
      printf("y-axis self test: acceleration trim within : "); printf("%0.1f",SelfTest[1]); printf("%% of factory value\n\r");
      printf("z-axis self test: acceleration trim within : "); printf("%0.1f",SelfTest[2]); printf("%% of factory value\n\r");
      printf("x-axis self test: gyration trim within : "); printf("%0.1f",SelfTest[3]); printf("%% of factory value\n\r");
      printf("y-axis self test: gyration trim within : "); printf("%0.1f",SelfTest[4]); printf("%% of factory value\n\r");
      printf("z-axis self test: gyration trim within : "); printf("%0.1f",SelfTest[5]); printf("%% of factory value\n\r");
    }
...
//    Serial.print("GYRO BIASES: "); Serial.print(gyroBias[0]); Serial.print("\t"); Serial.print(gyroBias[1]); Serial.print("\t"); Serial.print(gyroBias[2]); Serial.print("\n");
    ...
    if (SerialDebug){
      printf("AK8963 "); printf("I AM "); printf("0x%X",d); printf(" I should be "); printf("0x%X\n\r",0x48);
    }
  ...
  if(SerialDebug) {
    ...
    printf("X-Axis sensitivity adjustment value "); printf("%0.2f\n\r",magCalibration[0]);
    printf("Y-Axis sensitivity adjustment value "); printf("%0.2f\n\r",magCalibration[1]);
    printf("Z-Axis sensitivity adjustment value "); printf("%0.2f\n\r",magCalibration[2]);
  }
...
//  With the MPU9250, it is possible to get gyro sample rates of 32 kHz (!), 8 kHz, or 1 kHz
  switch (gyro_dlpf){
    case 0: writeByte(MPU9250_ADDRESS, CONFIG, GYRO_DLPF_CFG_250HZ); break; //doesn't matter, because we're going to disable DLPF
    case 41: writeByte(MPU9250_ADDRESS, CONFIG, GYRO_DLPF_CFG_41HZ); break;
    case 92: writeByte(MPU9250_ADDRESS, CONFIG, GYRO_DLPF_CFG_92HZ); break;
    case 184: writeByte(MPU9250_ADDRESS, CONFIG, GYRO_DLPF_CFG_184HZ); break;
    case 250: writeByte(MPU9250_ADDRESS, CONFIG, GYRO_DLPF_CFG_250HZ); break;
    default:
      printf("Gyro DLPF command not recognized! Using 41 Hz...\n\r");
  ...
  switch (accel_dlpf){
    ...
    default:
      printf("Accel DLPF command not recognized! Using 41 Hz...\n\r");
...
//  writeByte(MPU9250_ADDRESS, I2C_MST_CTRL, c);
  ...
  // can join the I2C bus and all can be controlled by the Arduino as master
   //writeByte(MPU9250_ADDRESS, INT_PIN_CFG, 0x2);//0x22);
